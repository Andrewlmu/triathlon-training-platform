...
interface LabelContextType {
  ...
  isLoading: boolean;
  addLabel: (
    label: Omit<WorkoutLabel, 'id' | 'userId' | 'createdAt' | 'updatedAt'>
  ) => Promise<WorkoutLabel>;
...
export const LabelProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addLabel = async (
    labelData: Omit<WorkoutLabel, 'id' | 'userId' | 'createdAt' | 'updatedAt'>
  ): Promise<WorkoutLabel> => {
    if (!session?.user?.id) {
  ...
  const updateLabel = async (
    id: string,
    labelData: Partial<WorkoutLabel>
  ): Promise<WorkoutLabel> => {
    if (!session?.user?.id) {

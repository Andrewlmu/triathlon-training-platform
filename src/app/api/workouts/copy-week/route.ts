...
export async function POST(request: Request) {
  try {
    ...
    if (!data.sourceWeekStart || !data.targetWeekStart) {
      return NextResponse.json({ error: 'Source and target weeks are required' }, { status: 400 });
    }
    ...
    if (sourceWeekWorkouts.length === 0) {
      return NextResponse.json({ error: 'No workouts found in the source week' }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: 'Failed to copy workouts' }, { status: 500 });
  }
}
